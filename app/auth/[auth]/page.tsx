...
export default function Auth({ params }: any) {
    ...
    const handleFormSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (params.auth === 'signup' && formData.pwd !== formData.cnfpwd) {
    ...
    return (
        <div className="relative py-24 flex justify-center gap-24 items-center bg-gray-100">
            ...
            <div className="relative sm:max-w-sm w-full">
                <div
                    className={`bg-blue-400 shadow-lg w-full h-full rounded-3xl absolute transform transition-all ease-out duration-200 ${isHovered ? 'rotate-6' : '-rotate-6'
                        }`}
                ></div>
                <div
                    className={`bg-green-400 shadow-lg w-full h-full rounded-3xl absolute transform transition-all ease-out duration-200 ${!isHovered ? 'rotate-6' : '-rotate-6'
                        }`}
                ></div>
                <div
                    className="relative w-full h-full rounded-3xl px-8 py-10 bg-gray-100 shadow-md"
                        ...
                        {params.auth !== 'login' && (
                            <div>
                                <input
                                    ...
                                    type="text"
                                    placeholder="Name"
                                    className="pl-4 mt-1 block w-full border-none bg-gray-200 h-11 rounded-xl shadow-lg hover:bg-blue-100 focus:bg-blue-100 focus:ring-0"
                                />
                            </div>
                        )}
                        <div className="mt-7">
                        ...
                        <div className="mt-7">
                            <div className="flex justify-center items-center">
                                <label className="mr-2">
                                    {params.auth === 'login' ? "Don't" : 'Already'} have an account?
                                </label>
